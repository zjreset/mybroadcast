...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:@"查看全部" style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] autorelease];
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [self arrangeThumbs];
    [self.navigationItem.rightBarButtonItem setTitle:@"查看大图"];
...
{
    _isThumbViewShowing = NO;
    [self.navigationItem.rightBarButtonItem setTitle:@"查看全部"];
